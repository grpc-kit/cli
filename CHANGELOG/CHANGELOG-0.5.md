...
- 将 CLI 与新服务模板使用的 `protoc-gen-grpc-gateway`、`protoc-gen-openapiv2` 和 Docker include 源升级到 `v2.30.0`。生成项目的 grpc-gateway runtime 暂时保留 `v2.29.0`：v2.30.0 要求的 `otelgrpc v0.70.0` 删除了已发布 `grpc-kit/pkg v0.5.0` 仍使用的 `StreamClientInterceptor`，待 pkg 发布兼容版本后再同步升级 runtime。
