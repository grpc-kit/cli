...
命令只会更新首行带有完整 `Code generated by "grpc-kit-cli/...". DO NOT EDIT.` 标记、且存在内嵌兼容资产的已有文件；`scripts/generate.sh` 的标记位于第二行（首行是 shebang），0.5.1 之前的存量副本没有标记，迁移凭"路径精确 + 全文 SHA-256 匹配冻结历史版本"的内容证据整文件同步到最新服务模板并补上标记，本地修改过或内容不明的副本只会得到 `generate_script_modified` 人工核对提示而不被覆盖，文件缺失则不创建。命令不会修改 `go.mod`、业务源码或其他用户管理文件，也不会创建、删除或重命名文件。`Manual action` 表示仍需完成的 Go 版本、pkg、logrus/slog 或 context API 迁移，因此“托管文件已迁移”不等于“项目已通过编译验证”。根级 `upgrade`/`update` 不用于项目迁移；未来 CLI 自身更新保留 `self update` 语义。

旧服务的用户源码可使用 grpc-kit 共享技能 `migrate-logrus-to-slog` 进行 AI 辅助迁移；安装共享技能后入口为 `scripts/skills/skills/migrate-logrus-to-slog/SKILL.md`。该技能可以在用户明确授权后修改业务源码和依赖，但不会放宽 `project migrate` 的 marker 写入规则，迁移结果仍需经过 diff 审阅、生成、测试和构建。
